...
import { useUpdateDocumentation } from "../hooks/useDocumentation";
import { documentationListItemToFormState } from "../utils/documentationUtils";
...
export default function EditDocumentationModal({
  ...
  const [form, setForm] = useState<CreateDocumentationInput>(() =>
    documentationListItemToFormState(documentation),
  );
  const [errors, setErrors] = useState<DocumentationFieldErrors>({});
  const [mode, setMode] = useState<EditPreviewMode>("edit");
  const { mutate: updateDocumentation, isPending } = useUpdateDocumentation();

  function handleClose() {
    if (isPending) return;
    setForm(documentationListItemToFormState(documentation));
    setErrors({});
    setMode("edit");
    onClose();
  }
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    updateDocumentation(
      ...
      {
        onSuccess: () => {
          notify.success("Documentación actualizada correctamente.");
          handleClose();
        },
        onError: (err) => {
          notify.error(
            err.message || "No fue posible actualizar la documentación.",
          );
        },
      },
    );
  ...
  return (
    <Overlay
      ...
      description="Modificá el título o el contenido en Markdown. Los cambios se guardan al confirmar."
      size="xl"
